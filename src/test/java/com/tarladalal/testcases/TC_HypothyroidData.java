...
public class TC_HypothyroidData extends BaseClass {
	ExcelReader read;
	...
	public void getRecipeDetails() throws InterruptedException {
		...
		submitbtn.click();
		Thread.sleep(2000);
		WebElement hypoLink = driver.findElement(By.xpath("//a[@class='rcpsrch_suggest'][1]"));
		hypoLink.click();
		Thread.sleep(1000);
	}
...
//		System.out.println("Number of Pages:" + pagescount);
			...
			int cardcount = driver.findElements(By.xpath("//div[@class='recipelist']/article[@class='rcc_recipecard']"))
					.size();
			...
			for (int j = 1; j <= pagescount; j++) {
				try {
					// List of recipename
					List<WebElement> ReceipeList = driver.findElements(By.xpath("//span[@class='rcc_recipename']/a"));
					for (WebElement element : ReceipeList) {
						System.out.println(element.getText());
						recipeNameList.add(element.getText());
					}

					// List of RecipeId
					List<WebElement> recId = driver.findElements(By.xpath("//div[@class='rcc_rcpno']/span"));
					for (WebElement element : recId) {
						String recpIds = element.getText();
						recpIds = recpIds.split("\n")[0];
						recipeIdList.add(recpIds.replaceAll("[^0-9]", ""));
						System.out.println(recpIds);
					...
					for (int k = 1; k < ReceipeList.size(); k++) {
						try {
							...
							recUrlList.add(driver.getCurrentUrl());
							recpUrl.click();
							// String recpUrl = element.getAttribute("href");
							// recUrlList.add(recpUrl);
							// element.click();
							Thread.sleep(3000);
